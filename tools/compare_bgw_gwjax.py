...
def get_gwjax_kpoints(wfn_path: Path) -> np.ndarray:
    """Return LORRAX's full-BZ unfolded k-point list (crystal coords, shape (nk_tot, 3)).

    LORRAX's eqp0.dat is indexed by ``sym.unfolded_kpts`` (full-BZ, size N_kx*N_ky*N_kz),
    NOT the IRR ``wfn.kpoints`` list (size nkpts ≤ N_kx*N_ky*N_kz).  The two agree for
    on-axis k-points but diverge off-axis (e.g. for a 4×4×4 grid they agree at indices
    0,1,2 and disagree from index 3 onwards).  Matching BGW IRR k-points against
    ``wfn.kpoints`` silently gives wrong bands at off-axis k-points.
    """
    import sys
    src_path = Path("/global/homes/j/jackm/software/lorrax_A/src")
    if src_path.exists():
