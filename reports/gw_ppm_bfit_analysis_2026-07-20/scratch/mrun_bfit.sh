#!/bin/bash
# Pool runner for agent/gw-ppm-bfit-analysis. PYTHONPATH -> lorrax_A_ppm_bfit worktree.
# Uses the shared lx-alloc-jackm pool allocation via --overlap (node was idle).
# Usage: NGPU=4 EXTRA_ENV="--env=LORRAX_BFIT_DUMP=/path ..." mrun_bfit.sh <workdir> <cmd...>
set -uo pipefail
JID=$(squeue -u jackm -h -o "%.12i %j" | awk '/lx-alloc-jackm/{print $1; exit}')
[ -z "${JID:-}" ] && { echo "NO_LXALLOC_POOL"; exit 3; }
NGPU="${NGPU:-4}"
WD="$1"; shift
WT=/pscratch/sd/j/jackm/lorrax_sandbox/sources/worktrees/lorrax_A_ppm_bfit
...
srun --jobid=$JID --overlap --immediate=120 -N1 -n"$NGPU" \
  --gres=gpu:"$NGPU" --cpus-per-task=16 --cpu-bind=cores --chdir="$WD" $SEL \
  shifter --image="$IMAGE" --module=gpu,mpich \
    ...
    --env=JAX_COMPILATION_CACHE_DIR="$JAXCACHE" --env=XLA_PYTHON_CLIENT_PREALLOCATE=false \
    --env=XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 --env=MPLBACKEND=Agg --env=OMP_NUM_THREADS=16 \
    ${EXTRA_ENV:-} \
    $INC \
    "$@"
