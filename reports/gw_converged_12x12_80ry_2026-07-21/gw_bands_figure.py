...
B0, B1 = (int(v) for v in os.environ.get("BF_WIN", "0:80").split(":"))
NVAL = int(os.environ.get("BF_NVAL", "26"))
# Absolute 0-based index of the first band in the htransform window.
# initialize_wfns builds the basis over (nelec-nval, nelec+ncond), so a
# gap-centred window (nval=8, ncond=8 -> bands 18..34) starts at 18, and the
# eqp*.dat lookup has to be offset by it: window index b is absolute band
# BOFF+b (0-based) = BOFF+b+1 (the 1-based label eqp uses).
BOFF = int(os.environ.get("BF_BOFF", "0"))
NCOND = int(os.environ.get("BF_NCOND", "54"))
NBAND = NVAL + NCOND
...
sub = (f"12×12 k-grid · 80 Ry · {NBAND} bands Σ-corrected · 326 bands "
       f"screening · 2412 centroids · {B1-B0}-band htransform")
