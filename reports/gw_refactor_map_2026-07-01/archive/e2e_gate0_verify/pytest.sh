...
echo "=== branch $(git branch --show-current) — unit suite (excl. slow e2e regression) ==="
LORRAX_NGPU=1 lxrun python3 -m pytest -q tests -m "not regression" -p no:cacheprovider 2>&1 | tail -25
