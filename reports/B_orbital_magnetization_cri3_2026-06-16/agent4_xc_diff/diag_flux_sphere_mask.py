"""DECISIVE: QE's GGA flux divergence (fft_graddot) sums ONLY over the ngm sphere
(|G|^2 <= ecutrho).  The flux field h = df/dsigma * grad(rho) is a REAL-SPACE
PRODUCT, so it has G-content BEYOND ecutrho (convolution widens support).  QE
DISCARDS that out-of-sphere content (it only loops dfft%ngm).  LORRAX takes the
divergence over the FULL FFT box (all G), KEEPING the out-of-sphere flux content.

For sharp (high-G) bands localized at the Cr core, this out-of-sphere flux is the
largest -> band-dependent residual on exactly bands 28/35/63.

Test: mask the GGA-flux field h to the ecutrho sphere (zero |G|>ecutrho) before
the divergence, in BOTH Vbar and B.  This reproduces QE's fft_graddot.  Re-measure.

Variants (full <v|H|v>-eps):
  base       : current LORRAX (flux divergence over full box)
  flux_sphere: flux field h masked to ecutrho sphere before divergence (QE-faithful)
"""
import os, sys
...
sphere = (G2 <= ecutrho).astype(jnp.complex128)
print(f"ecutrho={ecutrho:.1f} Ry; sphere cells={int(jnp.sum(sphere.real))}/{nx*ny*nz}")
...
def E_lda(uu, dd):
    z = jnp.zeros_like(uu); return jnp.sum((uu+dd)*pbe_xc_spin(uu, dd, z, z, z))
dfu = jax.grad(E, 0)(nu, nd, suu, sud, sdd); dfd = jax.grad(E, 1)(nu, nd, suu, sud, sdd)
dfu_l = jax.grad(E_lda, 0)(nu, nd); dfd_l = jax.grad(E_lda, 1)(nu, nd)
...
def assemble(mask_flux):
    Vu = dfu - div([2.0*duu*gnu[i] + dud*gnd[i] for i in range(3)], mask_flux)
    Vd = dfd - div([2.0*ddd*gnd[i] + dud*gnu[i] for i in range(3)], mask_flux)
...
def resid(Vbar_, B_, label, vb=False):
    rs = []; pb0 = None
    for ik in (0, 4, 8):
...
print("\n=== base: flux divergence over FULL box (current LORRAX) ===")
rb = resid(*assemble(False), "base", vb=True)
print("=== flux_sphere: GGA-flux field masked to ecutrho sphere (QE fft_graddot) ===")
rs2 = resid(*assemble(True), "flux_sphere", vb=True)
print("\nband-wise change base->flux_sphere (k0, meV):")
for b in (0, 28, 29, 35, 39, 40, 63, 69):
    print(f"   band{b}: {rb[b]:+.2f} -> {rs2[b]:+.2f}  (delta {rs2[b]-rb[b]:+.2f})")
