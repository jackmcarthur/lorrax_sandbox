...
import sys as _s
USE_SEGNI = ('--segni' in _s.argv)
seg = jnp.where(amag>1e-12, jnp.sign(-mzz), 1.0) if USE_SEGNI else jnp.ones_like(amag)
ZERO_CORE = ('--zerocore' in _s.argv)
n_for_xc = rho_val if ZERO_CORE else n          # vxc.dat uses vxc_zero_rho_core=.TRUE. (val only)
print(f"  [ZERO_CORE={ZERO_CORE}]")
nu=(n_for_xc+seg*amag)/2; nd=(n_for_xc-seg*amag)/2
V_up, V_dn = compute_V_xc_spin(nu, nd, jnp.fft.fftn(nu), jnp.fft.fftn(nd), G_cart)
Vbar = 0.5*(V_up+V_dn); Bmag = 0.5*(V_up-V_dn)
inv = jnp.where(amag > 1e-12, 1.0/(amag+1e-30), 0.0)
B_vec = jnp.stack([seg*Bmag*mxx*inv, seg*Bmag*myy*inv, seg*Bmag*mzz*inv], axis=0)
print(f"  [USE_SEGNI={USE_SEGNI}]")
