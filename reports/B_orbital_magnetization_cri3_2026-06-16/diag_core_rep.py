"""Does the NLCC core representation in V_xc cause the residual?
LORRAX uses an analytic 'precise-core' (subtract gridded, add rho_core_G);
QE uses the gridded core directly. Test both in the B_xc gate."""
import os, sys
...
def resid_for(rgu, rgd, label):
    ...
    for ik in (0, 4):
        ...
        print(f"  [{label}] k={ik}: max={r.max():.1f} mean={r.mean():.1f} band0={r[0]:.1f} VBM={r[nocc-1]:.1f}")

print("=== precise-core (analytic, current LORRAX) ===")
resid_for(jnp.fft.fftn(n_up - core_grid/2) + rho_core_G/2,
          jnp.fft.fftn(n_dn - core_grid/2) + rho_core_G/2, "precise")
print("=== gridded-core (like QE) ===")
resid_for(jnp.fft.fftn(n_up), jnp.fft.fftn(n_dn), "gridded")
