#!/bin/bash
# Module-free srun+shifter runner for the bse_k_grid worktree (agent/bse-kgrid).
# Mirrors run10.sh but PYTHONPATH -> lorrax_A_bse_kgrid/src.
#   usage: JID=<jobid> NGPU=<1|4> ./run_kgrid.sh <workdir> <python args...>
set -euo pipefail
JID="${JID:?set JID to the salloc/overlap job id}"
NGPU="${NGPU:-1}"
WD="$1"; shift

IMAGE="nvcr.io/nvidia/jax:25.04-py3"
SRC=/pscratch/sd/j/jackm/lorrax_sandbox/sources/worktrees/lorrax_A_bse_kgrid/src
