# Round 2 — Agent 4: critique + revision

You finished round 1 and wrote `reports/zeta_rchunk_memory_model_2026-05-13/agent_4.md`. The other three agents wrote `agent_1.md`, `agent_2.md`, `agent_3.md` in the same directory, independently.

**Now read those three files.** Then write `reports/zeta_rchunk_memory_model_2026-05-13/agent_4_v2.md` containing the five sections below. Be terse where you can, exhaustive where you must.

## 1. Cross-reading notes

For each of the other three drafts (agent_1, agent_2, agent_3), 5–15 lines:
...
Cite by `agent_M.md §N` and your own `agent_4.md §N`.
...
The set of items **all four of us should treat as unresolved**. Cross-reference: if Agent 1's §X open question is the same thing as my §Y, say so explicitly. Promote any single-agent flagged uncertainty to a team-level open question if it's load-bearing.
...
- Read `agent_1.md`, `agent_2.md`, `agent_3.md` — those are the inputs to this round.
- Stop when `agent_4_v2.md` is written. Print "Agent 4 round 2 done".
