...
## LORRAX output-file map (post-2026-05-04)

The Σ-output layout changed in the 2026-05-04 refactor. Current reality:

| File | Contents | Writer |
|---|---|---|
| `eqp0.dat` / `eqp1.dat` | **BGW `eqp.dat` text format** (per-k block: `kx ky kz nspin·nb` header, then `ispin iband E_DFT_eV E_QP_eV` rows), one `# Generated by LORRAX …` provenance line prepended | `gw/eqp_bgw.py` (byte-identical to BGW `Sigma/sigma_main.f90:3175-3193` past the comment; strip it with `tail -n +2` for BGW binaries) |
| `sigma_diag.dat` | The per-band `sigX/sigC` (PPM) or `sigSX/sigCOH` (COHSEX) decomposition — the `n=… sigX=…` line grammar documented below, plus a trailing `Eo=` (mean-field energy) column | `gw/gw_output.py` → `file_io.sigma_output.write_sigma_to_file` |
| `sigma_freq_debug.dat` | Header-driven named columns (see next section) | — |

Everything in the section below headed "GN-PPM" / "Static COHSEX" now
describes **`sigma_diag.dat`**, NOT `eqp0.dat` — before 2026-05-04 that
decomposition WAS `eqp0.dat`, which is why the parsers keep their names.
Labels switch on mode (PPM: `sigX/sigC/sigXC/VH`; COHSEX:
`sigSX/sigCOH/sigTOT/VH`), and the `VH` column reads 0.000 by design when
`kin_ion.h5` was generated with the Hartree folded in.

## LORRAX: `sigma_diag.dat` (pre-2026-05-04: `eqp0.dat`)

Quasiparticle self-energy decomposition. One block per k-point. Format
...
def parse_eqp0_gn(path):
    """Parse GN-PPM sigma_diag.dat (pre-2026-05-04: eqp0.dat)
    -> {band_1idx: {'sigX': float, 'sigC': float}}."""
...
def parse_eqp0_cohsex(path):
    """Parse COHSEX sigma_diag.dat (pre-2026-05-04: eqp0.dat)
    -> {band_1idx: {'sigSX': float, 'sigCOH': float}}."""
    import re
    bands = {}
    for line in open(path):
        m = re.match(r'n=(\d+)\s+', line.strip())
...
## LORRAX: `sigma_freq_debug.dat`

Detailed frequency decomposition (PPM runs only, when
`sigma_freq_debug_output = true`).

**Current format (LORRAX ≥ 2026-06): the columns are HEADER-DRIVEN, not
positional.** The file carries `#` provenance lines ending in a
`# k n <colnames...>` column-name line; the column set varies by mode and
complex quantities split into `.Re`/`.Im` sub-columns. Parse with the
header-driven parser in `skills/compare/SKILL.md` §2c
(`parse_sigma_freq_debug_v2`) — never positional indices (a wrong-column
comparison at the 8 eV scale is on record,
`_archive/KNOWN_SANDBOX_ERRORS.md` 2026-07-02).

**The table below is LEGACY** — the fixed 13/14-column layout of
pre-refactor (≤ 2026-05) archived outputs only. Disambiguation: legacy
files have the fixed `sig_c_invld(0)` name in their header; current files
have the `# k n ...` named header line. Tab-separated columns:
...
**Note** (legacy-era): The header comment "sig_c(Edft) includes head" is
misleading — sig_c(Edft) does NOT include the head unless
`apply_head_diagonal = true`. See `_archive/KNOWN_SANDBOX_ERRORS.md`.
(`apply_head_diagonal` has since been REMOVED from `gw_config._DEFAULTS`;
its `false` behaviour is the permanent default — see
`docs/BGW_LORRAX_MATCHING.md` convention 7.)
