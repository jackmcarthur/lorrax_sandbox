...
def _split_target_args(raw_argv: list[str]) -> tuple[str, list[str]]:
    """Split our own argv vs the target's. We support the canonical form:
    ...
    module = raw_argv[idx + 1]
    target_args = raw_argv[idx + 2:]
    our_args = raw_argv[:idx]
    return module, target_args, our_args
...
def main() -> int:
    ...
    if args.no_trace:
        trace_ctx = contextlib.nullcontext()  # type: ignore[name-defined]
    else:
