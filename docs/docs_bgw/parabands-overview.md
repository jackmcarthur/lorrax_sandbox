...
calculations from *quartic* to *quasi-quadratic***, and reduces the algorithmic 
prefactor, with **speedups of 10-1000$\times$** for typical system sizes of 10's to 100's 
of atoms, with negligible error. The use of stochastic pseudobands is **HIGHLY RECOMMENDED**.

See the documentation for stochastic pseudobands in [`stochastic pseudobands`](pseudobands-overview.md).
...
by the SAPO code written by Georgy Samsonidze. Stochastic pseudobands was written
by Aaron R. Altman and Felipe H. da Jornada.
