...
- `centroids_file = centroids_frac_<N>.txt` where N = 8 × nband for standard accuracy,
  12 × nband for high accuracy. Generated by `centroid.kmeans_cli` (see Execute Workflow step 5a).
...
- `qp_solver = one_shot_dft` (standard G0W0; this is the default — the legacy
  `sigma_at_dft_energies` key is deprecated, see docs_gwjax/COHSEX_INPUT.md §3)
