...
ROOT = Path('/pscratch/sd/j/jackm/lorrax_sandbox/runs/Si_pseudobands/'
            '00_si_2x2x2_60Ry/cohsex_valcond_2026-04-18')
...
def collect():
    ...
    bgw_cache = {}
    for wfn in ('parabands', 'pseudo'):
        for nc in (60, 140, 208):
...
def plot_mae_vs_Nmu(records, out_path):
    """MAE(Σ) vs N_μ for each (WFN, n_cond). One line per (WFN, n_cond)."""
    from collections import defaultdict
    by_group = defaultdict(list)  # (wfn, ncond) → list of (Nmu, mae)
    for r in records:
        by_group[(r['wfn'], r['ncond'])].append((r['Nmu'], r['mae_prime']))
    fig, ax = plt.subplots(1, 1, figsize=(9, 6))
    colors = {
        'parabands': {60: '#1f77b4', 140: '#2ca02c', 208: '#9467bd'},
        'pseudo':    {60: '#ff7f0e', 140: '#d62728', 208: '#8c564b'},
    }
    markers = {'parabands': 'o', 'pseudo': 's'}
    for (wfn, nc), pts in sorted(by_group.items()):
        pts.sort()
        Ns, maes = zip(*pts)
        ax.semilogy(Ns, maes,
                    marker=markers[wfn], color=colors[wfn][nc], lw=2.0, ms=8,
                    label=f"{wfn} n_cond={nc}")
    ax.set_xlabel(r'$N_\mu$  (centroids)', fontsize=12)
    ax.set_ylabel(r'MAE$(\Sigma_{\mathrm{COHSEX}})$  vs BGW Sig$^\prime$  (eV)',
                  fontsize=12)
    ax.set_title('LORRAX vs BGW COHSEX convergence with centroid count\n'
                 '(val_cond pair mode, use_bgw_vcoul=true, bands 1-16, 4 k-pts)',
...
def plot_scatter(records, out_path):
    ...
    n_rows = 2   # parabands, pseudo
    n_cols = 3   # ncond = 60, 140, 208
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(15, 9),
                             sharex=False, sharey=False, squeeze=False)
    Nmu_colors = {400: '#1f77b4', 800: '#ff7f0e', 1200: '#2ca02c'}
    for i, wfn in enumerate(['parabands', 'pseudo']):
        for j, nc in enumerate([60, 140, 208]):
            ax = axes[i][j]
