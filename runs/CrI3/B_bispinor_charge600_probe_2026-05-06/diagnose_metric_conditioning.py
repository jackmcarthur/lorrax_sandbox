#!/usr/bin/env python3
"""Compare current-centroid and scalar-centroid ISDF metric conditioning."""
...
PROBE = ROOT / "runs/CrI3/B_bispinor_charge600_probe_2026-05-06"

WFN_PATH = BASE / "WFN.h5"
CENTROIDS = {
    "current600_gordon": ROOT / "runs/CrI3/B_nonbisp_baseline/centroids_frac_600_current.txt",
    "charge600_subset": PROBE / "centroids_frac_600_charge_subset.txt",
}


def _point_current_ratios(psi_y: jax.Array, nelec: int) -> dict[str, float]:
    """Same-point occupied gamma-current amplitude divided by charge."""
    ...
    for mu in (1, 2, 3):
        ...
        out[f"gamma{mu}_ratio_max"] = float(np.max(ratio))
    return out
...
def _metric_stats(label: str, cfile: Path, wfn: WFNReader, sym: SymMaps, mesh: Mesh) -> None:
    ...
    print(f"nmu={nmu} band_range=(0,150) nk={meta.nk_tot} fft_grid={meta.fft_grid}", flush=True)
    ...
    # compute_CCT_from_left_right donates both operands, so the symmetric
    # diagnostic still needs distinct buffers.
...
def main() -> None:
    ...
    for label, cfile in CENTROIDS.items():
