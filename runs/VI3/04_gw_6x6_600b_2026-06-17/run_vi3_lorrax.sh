#!/bin/bash -l
# LORRAX stages on the live allocation: centroids -> dipole -> kin_ion -> GW.
set -u
...
die(){ echo "[lrx] FAILED: $1" >&2; exit 1; }
cd $RUN; ln -sf qe/nscf/WFN.h5 WFN.h5
echo "[lrx] centroids 4000 $(date)"
srun $J -N1 -n1 --gres=gpu:1 $SEL $SHIFTER $INC python3 -u -m centroid.kmeans_cli 4000 --seed 42 --oversample 1.0 > cent.out 2>&1
[ -s centroids_frac_4000.txt ] || die centroids
echo "[lrx] dipole $(date)"
srun $J -N1 -n1 --gres=gpu:1 $SEL $SHIFTER $INC python3 -u -m psp.get_dipole_mtxels -i $RUN/cohsex.in > dipole.out 2>&1
echo "[lrx] kin_ion $(date)"
...
[ -s eqp0.dat ] && [ -s sigma_mnk.h5 ] && echo "[lrx] GW DONE $(date)" || die gw_jax
