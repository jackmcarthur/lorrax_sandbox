#!/bin/bash
# Module-free srun+shifter runner for the bands-80 worktree (agent/bse-bands-80).
# Mirrors run 08's run_wt4.sh (KNOWN_SANDBOX_ERRORS 2026-07-15: Lmod broken in
# scripted contexts) but PYTHONPATH -> lorrax_A_bands80/src.
#   usage: JID=<jobid> NGPU=<1|4> ./run10.sh <workdir> <python args...>
set -euo pipefail
...
SRC=/pscratch/sd/j/jackm/lorrax_sandbox/sources/worktrees/lorrax_A_bands80/src
...
srun --jobid="$JID" --overlap --immediate=120 -N1 -n1 --gres=gpu:"$NGPU" --cpus-per-task=16 --cpu-bind=cores \
  --chdir="$WD" \
