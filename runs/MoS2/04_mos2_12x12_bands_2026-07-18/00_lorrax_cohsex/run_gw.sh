#!/bin/bash
# Preprocessing (dipole, kin_ion; single GPU) + 4-GPU gw_jax COHSEX+do_screened.
# Module-free srun+shifter (Lmod broken in scripts, KSE 2026-07-15); mirrors
# skills/execute_workflow GWJAX steps 5-6 with the exciton-bands worktree src.
...
srun --jobid=$JID --overlap --gres=gpu:1 -N 1 -n 1 --chdir="$RD" $SEL \
  shifter "${SHIFTER_ARGS[@]}" $INC \
