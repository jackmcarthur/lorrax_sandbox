...
# interpolation-uncertainty markers: window/basis A/B vs the committed v3
# (640c val(20,28)+cond(26,34)) — points where the two independent window
# choices disagree by > 50 meV are maximal-distance off-grid rows (mostly
# half-integer grid coordinates on the M-K leg).  Valence enters the BSE
# only at ON-grid k (exact), so these are cosmetic for the pipeline.
try:
    sp_ref = np.loadtxt("sp_bands_640ref.dat")
    unc = np.abs(bands_ev - sp_ref[:, 5:15]) > 0.050
except OSError:
    unc = np.zeros_like(bands_ev, dtype=bool)
...
if unc.any():
    iq_u, ib_u = np.where(unc)
    ax1.scatter(x_path[iq_u], bands_ev[iq_u, ib_u], s=16, facecolors="none",
                edgecolors="0.45", lw=0.8, zorder=5,
                label="interp-uncertain (window A/B > 50 meV)")
...
ax2.plot(x_path, D_clean, lw=1.8, color=C_DMIN2,
         label=r"$D_\min(Q)$, clean-window floor")
...
fig.text(0.5, -0.03,
         ...
         "Panel 1 open circles: off-grid points where independent windows "
         "disagree > 50 meV (valence pair, M-K leg); the BSE uses valence "
         "only at on-grid k, where htransform is exact.",
