#!/bin/bash
# Module-free srun+shifter runner, 1 GPU 1 rank (Lmod broken in scripts,
# KSE 2026-07-15).  Copy of ../01_lorrax_exciton_bands/run_wt4.sh with
# --gres=gpu:1 and --cpus-per-task/OMP for host-BLAS stages (perf-agent
# lesson: srun steps without --cpus-per-task are core-throttled).
#   usage: JID=<jobid> ./run_1gpu.sh <workdir> <python args...>
set -euo pipefail
...
srun --jobid="$JID" --overlap -N1 -n1 --gres=gpu:1 --cpus-per-task=32 \
  --cpu-bind=cores --chdir="$WD" \
