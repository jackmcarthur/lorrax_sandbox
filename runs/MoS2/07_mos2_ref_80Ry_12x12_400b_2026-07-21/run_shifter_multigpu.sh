#!/bin/bash
# Shifter launcher for the converged 12x12 / 80 Ry MoS2 GW+BSE campaign.
# PYTHONPATH -> sources/worktrees/lorrax_gw_converged/src
#   (agent/gw-converged-campaign, branched from agent/gw-conduction-postfix @ b7654ee:
#    xi-floor d011a36 + replicated zeta-fit ca78008 + rank-truncation 23af6b9 +
#    D3h density-point-group orbit closure 1794518 + band-range centroids/zeta_rcond=1e-8)
#
#   usage: JID=<jid> [NNODES=1] [NTASKS=1] [GRES=1] [LX_SRC=...] [EXTRA_ENV="--env=A=B ..."] \
#          ./run_shifter.sh <workdir> <python args...>
set -uo pipefail
...
SRC="${LX_SRC:-/pscratch/sd/j/jackm/lorrax_sandbox/sources/worktrees/lorrax_gw_converged/src}"
