"""Which bands blow up, and why? Print per-v x norms and residuals."""
import sys
...
# Run CG, inspect per-v x norms
for m in [5, 10, 20, 50]:
    x, info = cg_posdef(apply_A, -b, precond=precond, tol=1e-6, max_iter=m)
    xnorm = np.asarray(jnp.sqrt(jnp.sum(jnp.abs(x)**2, axis=(1,2))))
    resnorm = np.asarray(info.res_norms)
    print(f"\nm={m:3d}:  bands with |x|>100 or NaN:")
    bad = np.where(~np.isfinite(xnorm) | (xnorm > 100))[0]
    for v in bad[:10]:
        print(f"   v={v} eps_v={float(eps_vk[v]):.3f}  ||x||={xnorm[v]:.2e}  res={resnorm[v]:.2e}")
    print(f"   total bad bands: {len(bad)}/{len(xnorm)}")
