...
srun --jobid="$JID" --overlap -N1 -n1 --gres=gpu:1 --cpu-bind=cores --chdir="$WD" \
  shifter --image="$IMAGE" --module=gpu,mpich \
    --volume="$NVHPC:/lorrax_nvhpc" --volume="$PHDF5:/lorrax_phdf5" --volume="$SLATE:/lorrax_slate" \
    --env=PYTHONPATH="$PYPATH" --env=HDF5_USE_FILE_LOCKING=FALSE --env=LD_LIBRARY_PATH="$LDLIB" \
    --env=LD_PRELOAD=/lorrax_slate/lib/libmpi_gtl_cuda.so.0 --env=MPICH_GPU_SUPPORT_ENABLED=1 \
    --env=JAX_COMPILATION_CACHE_DIR=/pscratch/sd/j/jackm/lorrax_sandbox/runs/MoS2/A_bse_w0_resolvent_2026-07-16/finite_q/jaxcache_fresh \
