...
def write_with(name, backend, path, A, mesh, mu, spec_desc="mu-sharded"):
    """IMPLICIT spelling (decisions.md 2026-08-04): the logical extent is
    stated once, to create_dataset, and the padded operand is handed over
    with no further argument.  The 3x3 matrix therefore certifies the
    DERIVED default on every writer x reader pair."""
    unlink(path)
...
def read_padded_sharded(backend, path, mesh, mu, mu_pad):
    ...
    """
    with SlabIO(path, mode="r", mesh=mesh, backend=backend) as io:
        # No valid_shape: the overhang past the dataset's own mu extent
        # comes back zero-filled by derivation.
    ...
    if OOB_KIND == "write":
        ...
        try:
            with SlabIO(path, mode="w", mesh=mesh,
                ...
                sys.stdout.flush()
                # No global_shape: the dataset exists, and stating one
                # that disagrees with it is refused for that reason
                # instead.  The subject is the valid_shape OVERRUN.
